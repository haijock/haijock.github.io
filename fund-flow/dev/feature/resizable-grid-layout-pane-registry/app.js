 ...
 *   Pane registry for modular, self-registering analysis panes.
 *
...
import { initPanes, refreshPanes } from './core/pane-registry.js';
...
const FundFlow = {
    ...
    init() {
        ...
        this.bindEvents();
        this.initChart();
        initPanes(this);        // create pane DOM in #gridStack before gridstack init
        this.initGridstack();   // gridstack discovers all widgets including panes
        this.render();
    ...
    _defaultLayout: [
        { id: 'chart',      x: 0, y: 0,  w: 12, h: 5 },
        { id: 'toggles',    x: 0, y: 5,  w: 12, h: 1, noResize: true, noMove: true },
        { id: 'events',     x: 0, y: 6,  w: 6,  h: 24 },
        { id: 'priority',   x: 6, y: 6,  w: 6,  h: 6 },
        { id: 'milestones', x: 6, y: 12, w: 6,  h: 9 },
        { id: 'breakdown',  x: 6, y: 21, w: 6,  h: 5 },
        { id: 'cashflow',   x: 6, y: 26, w: 6,  h: 4 },
    ],
    ...
    initGridstack() {
        const grid = GridStack.init({
            ...
            handle: '.chart-header, .list-header, .pane-toggle-bar',
        ...
        if (saved) {
            try {
                ...
                console.error('Failed to restore grid layout:', e);
            }
        }
        ...
        // Resize charts when items are resized
        grid.on('resizestop', () => {
            this._resizeCharts();
        });
    },
    ...
    _saveGridLayout() {
        ...
        // Deduplicate by id — keep only the first entry per widget id.
        // Gridstack can accumulate phantom entries for dynamically
        // added/removed pane widgets across toggle cycles.
        const seen = new Set();
        ...
        localStorage.setItem('fundflow-layout', JSON.stringify(unique));
    },

    _resizeCharts() {
        setTimeout(() => {
            // Resize every active Chart.js instance (main chart + pane charts)
            Object.values(Chart.instances).forEach(c => c.resize());
        }, 50);
    },

    resetGridLayout() {
        localStorage.removeItem('fundflow-layout');
        localStorage.removeItem('fundflow-visible-panes');
        location.reload();
    },
...
// Expose FundFlow on window — needed by inline onclick handlers in rendered HTML
// and by Chart.js plugins that reference `typeof FundFlow !== 'undefined'`.
window.FundFlow = FundFlow;

// NOTE: FundFlow.init() is NOT called here. It is called by a final
// <script type="module"> in index.html, after all pane modules have
// registered via registerPane(). This ensures the registry is populated
// before initPanes() runs during init().
