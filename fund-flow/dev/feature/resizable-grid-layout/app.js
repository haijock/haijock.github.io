...
const FundFlow = {
    ...
    _defaultLayout: [
        { id: 'chart',   x: 0, y: 0, w: 12, h: 4 },
        { id: 'toggles', x: 0, y: 4, w: 12, h: 1, noResize: true, noMove: true },
        { id: 'events',  x: 0, y: 5, w: 12, h: 6 }
    ],
    ...
    _saveGridLayout() {
        ...
        localStorage.setItem('fundflow-layout', JSON.stringify(items));
    },
    ...
    resetGridLayout() {
        ...
        // Remove all toggled panes
        ['breakdown', 'priority', 'cashflow', 'milestones'].forEach(pane => {
            if (this.visiblePanes[pane]) {
                this.visiblePanes[pane] = false;
            }
        });
        location.reload();
    },
    ...
    togglePane(pane) {
        ...
        if (this.visiblePanes[pane]) {
            // Add pane to grid from template
            const tpl = document.getElementById('tpl-' + pane);
            if (tpl && this._grid) {
                const clone = tpl.content.firstElementChild.cloneNode(true);
                // Check saved layout for this pane's position
                const saved = localStorage.getItem('fundflow-layout');
                let opts = { w: 6, h: 4, id: pane, autoPosition: true };
                if (saved) {
                    try {
                        const items = JSON.parse(saved);
                        const match = items.find(i => i.id === pane);
                        if (match) opts = { ...match, id: pane };
                    } catch (e) { /* use defaults */ }
                }
                this._grid.addWidget(clone, opts);
                this._saveGridLayout();

                // Re-bind priority mode buttons if priority pane
                if (pane === 'priority') {
                    document.querySelectorAll('.pane-mode-btn').forEach(btn => {
                        btn.addEventListener('click', (e) => {
                            this.priorityMode = e.target.dataset.mode;
            ...
            if (this._grid) {
                const el = this._grid.engine.nodes.find(n => n.id === pane);
                if (el && el.el) {
                    this._grid.removeWidget(el.el, true);
                    this._saveGridLayout();
                }
            }
        }
